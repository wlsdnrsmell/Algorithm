...
public class No2477 {
	...
	//고객들의 접수 창구 대기 순서 정렬
	static void _sort(Cust arr[], int from, int to) {
		//버블정렬
		for(int i=from; i<to; ++i) {
			for(int j=i+1; j<=to; ++j) {
				//차량 정비소에 먼저 도착했거나, 도착시간이 같을 경우
				//고객 번호가 낮은 고객이 먼저 창구를 이용한다.
	...
	public static void main(String[] args) throws NumberFormatException, IOException {
		...
		for(int tc=1; tc<=T; tc++) {
			...
			for(int i=0; i<K; i++) {
				recq[i].t = Integer.valueOf(ti[i]);
				recq[i].k = i+1;
			}
			
			/*Sol sol = new Sol();
			Arrays.sort(recq, 0, K-1, sol);*/
	...
	static int solve() {
		...
		//지갑을 분실한 고객이 이용한 접수 창구와 정비 창구를 동일하게 이용한 고객이 없을때
		if (ans == 0 )
			return -1;
		
		//지갑을 분실한 고객이 이용한 접수 창구와 정비 창구를 동일하게 이용한 고객들의 번호 합
		else
			return ans;
	}
}
